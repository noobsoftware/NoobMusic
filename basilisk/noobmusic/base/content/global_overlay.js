...
var music_app = {
			...
			statement.executeAsync({
				...
				handleCompletion: function(aReason) {
					if(aReason != Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED) {
						...
						branch.set_audio("file://"+branch.root.settings.properties.library_location_input+branch.data[0].relative_filepath.split('#').join('%23'));
					}
				}
			});
		}
					...
					try {
						jsmediatags.read('file://'+file, {
							onSuccess: function(tag) {
								...
								/*if(typeof tag.tags.TSIZ !== 'undefined') {
									insert_values.size = tag.tags.TSIZ;
								}*/
	...
	playlist: {
		sidebar: {
			...
			add_to_playlist: function(song_id, playlist_id) {
				var branch = this;
		...
		reload_playlist: function(reload_all) {
			var branch = this;
			...
			if(typeof reload_all !== 'undefined') {
				if(typeof this.last_load_id != null && branch.root.playlist.last_load_id != -1) {
					this.load(this.last_load_id);
				} else if(branch.root.playlist.last_load_id != -1) {
					this.load();
				}
			...
			statement.executeAsync({
				...
				handleCompletion: function(aReason) {
					if(aReason != Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED) {
						...
						branch.sort(null, null, null, reset_order_cancel); //{ id: "order" }
					}
				}
			});
		...
		save_order: function() {
			...
			if(branch.last_load_id != null && branch.last_load_id != -1) {
				for(var x in branch.table) {
					var id = branch.table[x].id;
					var order = branch.table[x].song_order;
					var insert_values = {
						song_id: parseInt(id),
